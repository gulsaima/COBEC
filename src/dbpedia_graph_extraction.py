...
p3 = []
refdict = {}
def read_initialdict():
    """
    This function will read unigram, bigram and trigram concepts from disk and attach them as seperate lists
    :return:
    """
    concepts_list = []
    with open(r"..\temp\unigram_concepts" + ".json") as f:
        words = f.read().split()
        concepts_list.append(words)
    temp_list=[]
    with open(r"..\temp\bigram_concepts" + ".json") as f:
        ...
        concepts_list.append(temp_list)
    temp_list=[]
...
def read_refdic(p):
    global refdict
    with open(p / "initial_context.json") as json_file:
